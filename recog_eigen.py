    ...
    def show_video(self):
        ...
        while True:
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                # img_no = sorted([int(fn[:fn.find('.')]) for fn in os.listdir('test') if fn[0] != '.'] + [0])[-1] + 1
                # cv2.imwrite('test/%s.png' % (img_no),frame)
    ...
    def process_image(self, inImg):
        ...
        for i in range(len(faces)):
            ...
            if confidence[1]<3500:
                ...
                cv2.putText(frame, '%s - %.0f' % (person, confidence[1]), (x - 10, y - 10), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)

                self.orang.append(person)
                if(len(self.orang)==3):
                    if (self.orang[0]==self.orang[1] and self.orang[0]==self.orang[2] ):
                        for db in self.mycol.find({'nama' : self.orang[0], 'tanggal': str(self.wk.strftime("%d/%m/%Y")), 'tipe' : 'Eigenface' }):
                            self.data.append(db)

                        if len(self.data)==0:
                            self.mycol.insert_one({"nama" : self.orang[0], "waktu" : str(self.wk.strftime("%X")),"tanggal" : str(self.wk.strftime("%d/%m/%Y")),"tipe" :"Eigenface"})
                            self.root.withdraw()
                            messagebox.showinfo('Informasi',''+self.orang[0]+' Telah Presensi Tekan OK dan Tekan q untuk keluar')
                            self.data = []
                            self.root.destroy()
                            # self.root.quit()
                if(len(self.orang)>3):
